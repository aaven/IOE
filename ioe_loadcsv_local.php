<?php
    ...
    function importCSV(){
		...
		{
			die ('<p/>FAIL to empty $temptable: ' . mysqli_error($con));
		}
        
        $filename = $_GET["filename"];
        echo ("<p/>$filename");
        $filename = "wharton_data/AAPL.csv";
		...
		{
			die ('<p/>FAIL to load data into $te $temptable: ' . mysqli_error($con));
		}

        if (!mysqli_query($con,"DELETE FROM $temptable WHERE open=''"))
		{
			die('<p/>FAIL to filter data in $temptable: ' . mysqli_error($con));
		}
        ...
        if (!$result)
		{
            die ('<p/>FAIL to select cid from $temptable: ' . mysqli_error($con));
		}
        ...
        if ($c > 0)
        {
            ...
            while ($row = mysqli_fetch_row($result)) {
                ...
                if (!$res)
                {
                    die ('<p/>FAIL to select cname from $temptable: ' . mysqli_error($con));
                }
                ...
                while ($r = mysqli_fetch_row($res)) {
                    ...
                    if (!$ress)
                    {
                        die ('<p/>FAIL to select min(date),max(date) from $temptable: ' . mysqli_error($con));
                    }
